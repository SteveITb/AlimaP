...
//  v3: Edit modal fully functional, CV upload, photo upload
// ─────────────────────────────────────────────────────────────────────────────
...
function uploadCV(event) {
  ...
  if (file.type !== 'application/pdf') {
    showEl(fb, '✗ Please upload a PDF file only.', '#a32d2d'); return;
  }
  if (file.size > 10 * 1024 * 1024) {
    showEl(fb, '✗ File too large. Max 10 MB.', '#a32d2d'); return;
  }
  showEl(fb, 'Uploading CV…', '#185fa5');
  const reader = new FileReader();
  reader.onload = e => {
    localStorage.setItem(CV_KEY, e.target.result);
    initCVButton();
    showEl(fb, '✓ CV uploaded successfully! Visitors can now download it.', '#0f6e56');
...
function deleteCV() {
  ...
  showEl(document.getElementById('cv-upload-feedback'), 'CV removed.', '#a32d2d');
}
...
function uploadProfilePhoto(event) {
  ...
  reader.onload = e => {
    ...
    document.getElementById('hero-photo-preview').src = e.target.result;
    document.getElementById('hero-photo-preview-wrap').style.display = 'block';
    showEl(fb, '✓ Photo updated on the homepage!', '#0f6e56');
  };
  reader.readAsDataURL(file);
}
...
function addProject() {
  ...
  Promise.all(projImageFiles.map(readFile)).then(images => {
    const projects = loadProjects();
...
// ── EDIT MODAL ────────────────────────────────────────────────────────────────
// KEY FIX: close admin first, then open modal after a short delay
function openEditModal(id) {
  ...
  document.getElementById('edit-proj-level').value  = p.level  || 'national';
  ...
  if (p.images && p.images.length) {
    ...
    existingWrap.innerHTML = '<p style="color:var(--txt-mute);font-size:.82rem;margin:0;">No photos yet — upload below.</p>';
  }

  // Reset new image section
  ...
  // Close admin panel first, then open modal (fixes z-index/display conflict)
  closeAdmin();
  setTimeout(() => {
...
function removeExistingImg(projectId, imgIdx) {
  ...
  saveProjects(projects);
  // Refresh the existing images display without closing modal
  const existingWrap = document.getElementById('edit-existing-imgs');
  if (p.images.length) {
    existingWrap.innerHTML = p.images.map((src, i) => `
      <div class="img-preview-item" id="eimg-${i}">
...
function removeNewEditImg(idx) {
  ...
  const el = document.getElementById(`eprev-${idx}`);
  if (el) el.remove();
}
...
function saveEdit() {
  ...
  Promise.all(editImageFiles.map(readFile)).then(newImages => {
    const projects = loadProjects();
    ...
    renderAll();
    refreshManageList();
    showFeedback(fb, '✓ Project updated successfully!', 'ok');
...
function refreshManageList() {
  ...
  list.innerHTML = projects.map(p => `
    <div class="project-list-item">
      <div class="pli-thumb">${p.images && p.images.length ? `<img src="${p.images[0]}" alt="">` : '📷'}</div>
...
function openAdmin() {
  const cvData = localStorage.getItem(CV_KEY);
  if (cvData) {
    document.getElementById('cv-filename').textContent = 'CV File (uploaded)';
    document.getElementById('cv-current-wrap').style.display = 'flex';
  }
  refreshManageList();
...
function submitContact() {
  ...
  ['contact-name','contact-email','contact-org','contact-msg'].forEach(id => {
